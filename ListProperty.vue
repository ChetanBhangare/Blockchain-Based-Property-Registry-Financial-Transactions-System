<template>
  <div class="view-container">
    ...
    <div class="view-content">
      ...
      <form @submit.prevent="listForSale" class="form-container">
        <div class="form-group">
          <label for="tokenId">Token ID:</label>
          <input type="number" id="tokenId" v-model.number="tokenId" required />
        </div>
        <div class="form-group">
          <label for="price">Sale Price (ETH):</label>
          <input type="number" id="price" v-model="price" step="0.01" placeholder="e.g., 2.0" required />
        </div>
    ...
    async listForSale() {
      ...
      try {
        ...
        const tx = await contract.listPropertyForSale(
          this.tokenId,
          parseEther(this.price.toString())
        );
        await tx.wait();
        this.success = "Property listed for sale successfully!";
        this.tokenId = '';
        this.price = '';
      } catch (err) {
        this.error = "Error listing property: " + err.message;
      } finally {
        this.loading = false;
      }
    }
