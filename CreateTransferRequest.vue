<template>
  <div class="view-container">
    ...
    <div class="view-content">
      ...
      <form @submit.prevent="createTransfer" class="form-container">
        <div class="form-group">
          <label for="tokenId">Token ID:</label>
          <input type="number" id="tokenId" v-model.number="tokenId" required />
        </div>
        <div class="form-group">
          <label for="recipient">New Owner Address:</label>
          <input type="text" id="recipient" v-model="recipient" placeholder="0x..." required />
        </div>
  ...
  methods: {
    ...
    async createTransfer() {
      ...
      try {
        ...
        const tx = await contract.createTransferRequest(this.tokenId, this.recipient);
        await tx.wait();
        this.success = "Transfer request created successfully!";
        this.tokenId = '';
        this.recipient = '';
      } catch (err) {
        this.error = "Error creating transfer request: " + err.message;
      } finally {
        this.loading = false;
      }
    }
