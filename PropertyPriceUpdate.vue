<template>
    <div class="view-container">
      ...
      <div class="view-content">
        ...
        <form @submit.prevent="updatePrice" class="form-container">
          <div class="form-group">
            <label for="tokenId">Token ID:</label>
            <input type="number" id="tokenId" v-model.number="tokenId" required />
          </div>
          <div class="form-group">
            <label for="newPrice">New Price (ETH):</label>
            <input type="number" id="newPrice" v-model="newPrice" step="0.01" placeholder="e.g., 1.75" required />
          </div>
    ...
    methods: {
      ...
      async updatePrice() {
        ...
        try {
          ...
          // Call updatePropertyPrice; assumes that the function validates price using the Oracle.
          const tx = await contract.updatePropertyPrice(this.tokenId, parseEther(this.newPrice.toString()));
          await tx.wait();
          this.success = "Property price updated successfully!";
          this.tokenId = '';
          this.newPrice = '';
        } catch (err) {
          this.error = "Error updating price: " + err.message;
          console.error(err);
        } finally {
          this.loading = false;
        }
