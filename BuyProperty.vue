<template>
  <div class="view-container">
    ...
    <div class="view-content">
      <h2>Buy Property</h2>
      <form @submit.prevent="buy" class="form-container">
  ...
  methods: {
    ...
    async buy() {
      ...
      try {
        ...
        // Fetch property details to determine the price (in wei)
        const property = await contract.getProperty(this.tokenId);
        const price = property[3];
        const tx = await contract.buyProperty(this.tokenId, { value: price });
        await tx.wait();
        this.success = "Property purchased successfully!";
        this.tokenId = '';
      } catch (err) {
        this.error = "Error buying property: " + err.message;
      } finally {
        this.loading = false;
      }
    }
